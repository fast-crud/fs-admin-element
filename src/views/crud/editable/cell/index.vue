    ...
    <fs-crud ref="crudRef" v-bind="crudBinding">
      <template #actionbar-right>
        ...
        <template v-if="crudBinding.table.editable.enabled">
          ...
          >
            <el-switch v-model="crudBinding.table.editable.exclusive" class="ml-5"> </el-switch>
          </fs-label>

          <fs-label label="排他式激活效果" class="ml-5" title="排他式激活时，将未关闭的自动保存还是取消">
            <el-radio-group v-model="crudBinding.table.editable.exclusiveEffect" class="ml-5">
              <el-radio-button label="cancel">自动取消</el-radio-button>
              <el-radio-button label="save">自动保存</el-radio-button>
            </el-radio-group>
          </fs-label>
